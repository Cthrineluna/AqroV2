...
const ScannerScreen = ({ navigation }) => {
  ...
  const handleBarCodeScanned = async (result) => {
    ...
    try {
      ...
      const response = await axios.post(`${getApiUrl('/containers/register')}`, { qrCode: data }, { headers: { Authorization: `Bearer ${token}` } });
  
      if (response.status === 200) {
        if (response.data.alreadyRegistered) {
          const message = response.data.ownedByCurrentUser
            ? 'This container is already registered to your account.'
            : 'This container is already registered to another user.';
  
          Alert.alert('Already Registered', message, [{ text: 'Try Again', onPress: resetScanState }]);
        } else {
          Alert.alert('Container Registered!', 'The container has been successfully registered to your account.', [
